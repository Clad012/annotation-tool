<template id="annotatable-template">
	<div class="annotatable" v-on:mousedown="beginDraw" v-on:mousemove="drawing" v-on:mouseup="drawn">
		<slot>Type</slot>
		<annotation v-for="(annotation, index) in annotations" :props="annotation" :input="input" :options="options" :key="index" 
					@remove="remove"
					@labeled="labeled"
					@toEdit="toEdit"
		>
...
export default {
	...
	methods: {
		...
		labeled: function(msg) {
			...
			for (var i = 0; i < this.annotations.length; i++) {
				if (this.annotations[i].id == msg.id) {
					...
					this.annotations[i].categorie = msg.categorie;
		...
		remove: function(msg) {
			...
			// });
			// this.lock = false;
			this.$emit('removeAnnotation', msg)
			this.lock = false;
		},
		toEdit: function(msg) {
			for (var i = 0; i < this.annotations.length; i++) {
	...
	created: function () {
		this.$on('labeled', this.labeled)
		this.$on('remove', this.remove)
		this.$on('toEdit', this.toEdit)
	},
