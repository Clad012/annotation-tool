<template>
	<div class="annotation" v-bind:style="size">
		...
		<component v-bind:style="inputPosition" v-show="editing" :is="input" :options="options" @edit="edit"></component>
		<a class="btn-close" href="#" @click.prevent="close" @mousedown.stop @mouseup.stop>&times;</a>
		<a v-if="showEditButton" class="btn-edit" href="#" @click.prevent="toEdit" @mousedown.stop @mouseup.stop>edit</a>
	</div>
</template>
<script>
...
export default {
	...
	methods: {
		labeled: function(e) {
			this.$emit('labeled', {id: this.id, label: e.target.value, categorie: e.target.value});
		},
		close: function(e) {
		...
		edit: function(msg) {
			this.$emit('labeled', {id: this.id, label: msg.label, categorie: msg.categorie});
		}
	},
	events: {
		edit: function(msg) {
